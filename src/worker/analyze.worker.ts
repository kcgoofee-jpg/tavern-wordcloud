...
import { analyze, analyzeAsync, prepareTexts, type AnalyzeOptions, type SourceFile } from '../core/analyze';
...
const report = (p: Omit<WorkerProgress, 'progress'>) =>
  self.postMessage({ ...p, progress: true } as WorkerProgress);
...
async function handle(req: WorkerRequest): Promise<void> {
  try {
    if (req.kind === 'cancel') {
      job?.abort.abort();
      self.postMessage({ id: req.id, ok: true, kind: 'load', ...summarize() } as WorkerResponse);
      return;
    }
    ...
    if (req.kind === 'samples') {
      ...
      self.postMessage({ id: req.id, ok: true, kind: 'samples', samples: raw.slice(0, req.n) } as WorkerResponse);
      return;
    }
    ...
    if (req.kind === 'context') {
      ...
      self.postMessage({ id: req.id, ok: true, kind: 'context', snippets } as WorkerResponse);
      return;
    }
    ...
    if (req.kind === 'curate') {
      ...
      const base = analyze(files, req.options, undefined, (done, total) =>
    ...
    self.postMessage({ id: req.id, ok: true, kind: 'analyze', result } as WorkerResponse);
  } catch (err) {
    self.postMessage({
      id: req.id,
      ok: false,
