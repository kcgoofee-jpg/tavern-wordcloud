...
export function baseForms(w: string): string[] {
  ...
  if (w.endsWith("'s") || w.endsWith('’s')) out.add(w.slice(0, -2));
  else if (w.endsWith("s'") || w.endsWith('s’')) out.add(w.slice(0, -1));
 ...
 */
export function detectEnglishNames(texts: readonly string[]): string[] {
  const CAP = /\b[A-Z][a-z]{1,20}\b/g;

  /** Positions of capitalized words in a sentence. */
  ...
  // Single-word names (Nicole, Maya): capitalized mid-sentence at least SINGLE_MIN times and
  ...
  const singles = [...midCount.entries()]
    .filter(([w, n]) => n >= SINGLE_MIN && w.length >= 3
