...
Everything in the local edition runs inside your browser and works offline. The interface is the same as the web version, except the entry line reads "Everything is processed on this computer" and there is no community board.
...
1. Drop SillyTavern exports onto the import page. Accepted:
   ...
   - a full backup `.zip` (recommended: lorebook and character-card keywords become a proper-noun dictionary, and regex scripts from the settings and cards are applied as cleaning rules)
...
3. Left rail, top to bottom: add files, filters & tokenization, word table, advanced, model endpoint, export, clear.
...
The key stays in your browser; the web version's relay forwards only the URL, the body and the authorization header and stores nothing. Full details in the README section "With your own API key".
...
**Why is a name split into pieces?** Names are unknown words; the tool discovers them statistically. Drop the full `.zip` so lorebook and card keywords act as a dictionary, or add the word under Advanced → forced words.

**Cannot load the model list / Failed to fetch.** Web: a base URL is fine (`/chat/completions` is appended), pick the model from the list. Local: providers without CORS fail on a direct browser call; use the web version or another provider.
